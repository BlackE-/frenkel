class MenuBeneficios{
    ...
    onIntersect = (entries) => {entries.forEach(entry => {if (entry.isIntersecting) {return this.setInViewStyles(entry.target)}return this.setOutOfViewStyles(entry.target)})}
    ...
    setDesktop = () =>{
        ...
        let boxChildren = this.sections[0].children;
        for(let i of boxChildren){i.classList.add('is-inview');}
    }
